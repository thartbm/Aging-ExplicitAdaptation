...
source('R/shared.R') # ?? do we need anything else?  color schema, and generic functions such as for confidence intervals
...
We would probably want to add $\eta^2_p$ to these t-tests at some point. But they are against 0, so they can't be translated to a one-way ANOVA.

## Do we have any evidence of declined proprioceptive acuity in the older participants?

Here comes a test that is largely copied from the Baseline project, and it uses only the aligned data.

```{r}
plotVarianceAge(target='svg')
```



Where we mention R, we should cite them:
